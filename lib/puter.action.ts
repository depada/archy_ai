...
const buildWorkerEndpoint = (path: string) => {
...
export const createProject = async ({
  ...
  try {
    const response = await puter.workers.exec(
      buildWorkerEndpoint("/api/projects/save"),
      {
        method: "POST",
        body: JSON.stringify({
          project: payload,
...
export const getProjects = async () => {
  ...
  try {
    const response = await puter.workers.exec(
      buildWorkerEndpoint("/api/projects/list"),
      { method: "GET" },
    );
...
export const getProjectById = async ({ id }: { id: string }) => {
  ...
  try {
    const response = await puter.workers.exec(
      buildWorkerEndpoint(`/api/projects/get?id=${encodeURIComponent(id)}`),
      { method: "GET" },
    );
